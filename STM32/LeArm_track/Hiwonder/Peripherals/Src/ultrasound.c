...
#define ULTRASOUND_I2C_READY_TIMEOUT_MS  10U
#define ULTRASOUND_I2C_RETRY_DELAY_MS     2U

UltrasoundHandleTypeDef ultrasound;

static uint8_t write_data(UltrasoundHandleTypeDef* self, uint8_t* pdata, uint16_t size)
{
	if(!i2c1_wait_ready(ULTRASOUND_I2C_READY_TIMEOUT_MS))
	{
		i2c1_recover();
		return 1;
	}
...
static bool write_to_device(UltrasoundHandleTypeDef* self, uint8_t reg, uint8_t* pdata, uint16_t size)
{
	...
	{
		HAL_Delay(ULTRASOUND_I2C_RETRY_DELAY_MS);
		i2c1_recover();
		if(write_data(self, trans_data, sizeof(trans_data)) != 0)
		{
			return false;
...
uint16_t get_ultrasound_distance()
{
	...
	if(!receive_from_device(&ultrasound, ULTRASOUND_DISTANCE_REG, distance, sizeof(distance)) ||
	   !i2c1_wait_ready(ULTRASOUND_I2C_READY_TIMEOUT_MS))
	{
		i2c1_recover();
