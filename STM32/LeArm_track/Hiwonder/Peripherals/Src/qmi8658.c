...
#define QMI8658_I2C_READY_TIMEOUT_MS  10U
#define QMI8658_I2C_RETRY_DELAY_MS     2U

QMI8658HandleTypeDef qmi8658;

static uint8_t write_data(QMI8658HandleTypeDef* self, uint8_t* pdata, uint16_t size)
{
	if(!i2c1_wait_ready(QMI8658_I2C_READY_TIMEOUT_MS))
	{
		i2c1_recover();
		return 1;
	}
...
static bool write_to_device(QMI8658HandleTypeDef* self, uint8_t reg, uint8_t* pdata, uint16_t size)
{
	...
	{
		HAL_Delay(QMI8658_I2C_RETRY_DELAY_MS);
		i2c1_recover();
		if(write_data(self, trans_data, sizeof(trans_data)) != 0)
		{
			return false;
...
void qmi8658_get_accelerometer(float* acc)
{
	...
	{
		if(!receive_from_device(&qmi8658, ACCEL_XOUT_H + i, &qmi8658.rec_data[i], 1) ||
...
void qmi8658_get_gyro(float* gyro)
{
	for(uint8_t i = 0; i < 6; i++)
	{
		if(!receive_from_device(&qmi8658, GYRO_XOUT_H + i, &qmi8658.rec_data[i], 1) ||
		   !wait_receive_complete())
		{
			return;
		}
...
void qmi8658_get_euler(float* euler)
{
	for(uint8_t i = 0; i < 6; i++)
	{
		if(!receive_from_device(&qmi8658, EULER_XOUT_H + i, &qmi8658.rec_data[i], 1) ||
		   !wait_receive_complete())
		{
			return;
		}
